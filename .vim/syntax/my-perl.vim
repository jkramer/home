...
if exists("perl_fold") && exists("perl_fold_blocks")
  ...
  syn keyword perlConditional		if elsif unless when default
...
if exists("perl_fold") && exists("perl_fold_blocks")
  ...
  syn keyword perlRepeat		while for foreach do until continue given
endif
...
syn keyword perlStatementStorage	my local our state has extends
...
syn match   perlStatementInclude	"\<\(use\|no\)\s\+\(\(integer\|strict\|less\|lib\|sigtrap\|subs\|vars\|warnings\|utf8\|byte\|base\|fields\|feature\|overload\|constant\)\>\)\="
...
syn keyword perlStatementMisc		warn formline reset scalar delete prototype lock confess carp croak
...
" syn match  perlVarPlain          "$[a-zA-Z_][0-9a-zA-Z_]*->[a-zA-Z_][a-zA-Z0-9_]*"
...
"if exists("perl_extended_vars")
  ...
  syn region perlVarBlock	matchgroup=perlVarPlain start="&\$*{" skip="\\}" end="}" contains=@perlExpr nextgroup=perlMethod
  ...
  syn region perlVarMember	matchgroup=perlVarPlain start="\(->\)\=\[" skip="\\]" end="]" contained contains=@perlExpr nextgroup=perlVarMember,perlVarSimpleMember,perlMethod
    syn match  perlMethod		"\(->\)\I\i*\(([^)]*)\)\?" contained contains=@perlExpr nextgroup=perlMethod,perlVarMember,perlVarSimpleMember
" syn match  perlMethod		"\(\(\(([^)]*)\)\?->\)\I\i*\)\+" contained contains=@perlExpr,perlMethod
"endif
